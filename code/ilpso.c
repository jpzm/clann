                 ...
                 const unsigned int particle_length)
{
    ...
    matrix_initialize(&s->positions,
                      s->number_of_particles,
                      particle_length);
    matrix_initialize(&s->velocities,
                      s->number_of_particles,
                      particle_length);
    matrix_initialize(&s->best_positions,
                      s->number_of_particles,
                      particle_length);
...
{
    matrix_finalize(&s->positions);
    matrix_finalize(&s->velocities);
    matrix_finalize(&s->best_positions);

    free((void *) s->fitness);
    free((void *) s->solution);
}
          ...
          const clann_real_type noticeable_change_rate)
{
    ...
    for (i = 0; i < s->number_of_particles; i++)
    {
        s->f_constraint(s->problem,
                        matrix_value(&s->positions, i, 0));
        s->fitness[i] = s->f_fitness(s->problem,
                                     matrix_value(&s->positions, i, 0));
    }

    for (i = 0; i < s->particle_length; i++)
        s->solution[i] = *matrix_value(&s->positions, 0, i);

    s->last_best = matrix_value(&s->positions, 0, 0);
           ...
           change_count < noticeable_change_rate)
    {
        ...
        for (i = 0; i < s->number_of_particles; i++)
        {
            ...
            v = s->f_fitness(s->problem,
                             matrix_value(&s->best_positions, i, 0));

            if (s->fitness[i] > v)
                for (c = 0; c < s->particle_length; c++)
                {
                    v = *matrix_value(&s->positions, i, c);
                    *matrix_value(&s->best_positions, i, c) = v;
                }
            ...
            if (clann_nrand() > iteration / (max_iterations))
            {
                ...
                while (c++ < (s->number_of_particles / 5))
                {
                    ...
                    for (j = 0; j < s->particle_length; j++)
                        avarage[j] += *matrix_value(&s->positions, v, j) / 5;
                }
            ...
            for (j = 0; j < s->particle_length; j++)
            {
                v = *matrix_value(&s->velocities, i, j);

                w = *matrix_value(&s->best_positions, i, j);
                w = w - *matrix_value(&s->positions, i, j);
                v += s->acceleration_coefficients[0] * r1 * w;

                w = reference[j] - *matrix_value(&s->positions, i, j);
                ...
                *matrix_value(&s->velocities, i, j) = v;
                *matrix_value(&s->positions, i, j) += v;
            }
            ...
            if (s->fitness[i] > local_best_fitness)
            {
                ...
                {
                    for (c = 0; c < s->particle_length; c++)
                        s->solution[c] = *matrix_value(&s->positions, i, c);

                    best_fitness = s->fitness[i];
                }

                local_best_fitness = s->fitness[i];
                s->last_best = matrix_value(&s->positions, i, 0);
            }
        }
        ...
        if (ilpso_compute_probability_of_mutation(s) > clann_nrand())
        {
            ...
            while (c < s->number_of_particles / 5)
            {
                ...
                if (matrix_value(&s->positions, i, 0) != s->last_best)
                {
                    c++;
                    ...
                    for (j = 0; j < s->particle_length; j++)
                    {
                        ...
                        *matrix_value(&s->positions, i, j) = w;
                    }

                    s->f_constraint(s->problem,
                                    matrix_value(&s->positions, i, 0));

                    w = s->f_fitness(s->problem, matrix_value(&s->positions,
                                                              i,
                                                              0));
                ...
                if (s->fitness[i] > local_best_fitness)
                {
                    ...
                    {
                        for (j = 0; j < s->particle_length; j++)
                            s->solution[j] = *matrix_value(&s->positions,
                                                           i,
                                                           j);
...
ilpso_initialize_positions_and_velocities(struct ilpso *s)
{
    ...
    for (i = 0; i < s->number_of_particles; i++)
        for (j = 0; j < s->particle_length; j++)
        {
            ...
            *matrix_value(&s->positions, i, j) = v;
            *matrix_value(&s->velocities, i, j) = 0;
            *matrix_value(&s->best_positions, i, j) = v;
        }
}
